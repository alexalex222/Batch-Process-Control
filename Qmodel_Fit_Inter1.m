...
Q_model_Inter1.coeffs = beta_stored(:,:,nPC_idx);
Q_model_Inter1.y_index = desired_outputs;
Q_model_Inter1.x_index = [];
Q_model_Inter1.x0_index = desired_initial_states;
Q_model_Inter1.u_index = desired_inputs;
Q_model_Inter1.RMSE = RMSE;

Q_model_Inter1.f_index = [];
Q_model_Inter1.q_index = 1;
Q_model_Inter1.q_times = [];
Q_model_Inter1.nb_fit = nb_fit;
Q_model_Inter1.nb_val = nb_val;
Q_model_Inter1.nb_alt = (nb_test+nb_fit+nb_val) - nb_fit - nb_val;
Q_model_Inter1.nPC = PCs_to_try(nPC_idx);
Q_model_Inter1.sa = std(xs);
Q_model_Inter1.T95 = (nb_fit-1)*(nb_fit+1)*Q_model_Inter1.nPC/(nb_fit*(nb_fit-Q_model_Inter1.nPC))*finv(.95,Q_model_Inter1.nPC,nb_fit-Q_model_Inter1.nPC);
Q_model_Inter1.T99 = (nb_fit-1)*(nb_fit+1)*Q_model_Inter1.nPC/(nb_fit*(nb_fit-Q_model_Inter1.nPC))*finv(.99,Q_model_Inter1.nPC,nb_fit-Q_model_Inter1.nPC);
Q_model_Inter1.wstar = xl;
Q_model_Inter1.wstarAll = wstarAll;
Q_model_Inter1.Q = yl;
Q_model_Inter1.P = P;
Q_model_Inter1.W = W;
Q_model_Inter1.PAll = PAll;
Q_model_Inter1.OMEGA = Q_model_Inter1.wstar'*(x_fit_centred'*x_fit_centred)*Q_model_Inter1.wstar./(size(x_fit_centred,1)-1);

Q_model_Inter1.SPE95 = chi_95;
Q_model_Inter1.SPE99 = chi_99;

Q_model_Inter1.SPE_lim_calc = @(Prob) g*chi2inv(Prob, h);
Q_model_Inter1.T_lim_calc = @(Prob) (nb_fit-1)*(nb_fit+1)*Q_model_Inter1.nPC/(nb_fit*(nb_fit-Q_model_Inter1.nPC))*finv(Prob,Q_model_Inter1.nPC,nb_fit-Q_model_Inter1.nPC);

Q_model_Inter1.x_mean = x_fit_mu;
switch model_use
    case 'MultiMod'
        Q_model_Inter1.x_std = x_fit_sigma.*...
            [ones(1,size(Q_model_Inter1.x0_index,2))...
            ones(1,size(x_fit_sigma,2)-size(Q_model_Inter1.x0_index,2))*sqrt(size(x_fit_centred,2))];
    case 'MissingData'
        Q_model_Inter1.x_std = x_fit_sigma;
end
Q_model_Inter1.y_mean = qual_fit_mu;
Q_model_Inter1.y_std = qual_fit_sigma;



Q_model_Inter1.error = [0 0 0];

save(fname,'Q_model_Inter1');
%%
figure;
