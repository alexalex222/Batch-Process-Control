function [quality_result, q_old, x, y,q, u, JMPC, time] =...
          QBMPC_FinalState(bdb, batch_index, SSM, qual_target)
%Quality Relevant Btach MPC function for Penicillin process
%   Input
%   bdb: batch data
%   index: the index of batch to be used in the MPC
%   SSM: the model contains the subspace ID based dynamic model and quality
%   model
...
qual_target=qual_target_final.*[0.1 1.5 1.9];
...
%     num_meas = size(Q_model.x_index,2)+size(Q_model.y_index,2) + size(Q_model.u_index,2);
    ...
    U.name = 'U';
    for j = 1:size(SSM.u_typ_idx,2)*2
        U.uels{j} = j;
    end
    ...
    wgdx('QBMPCv2sets', T, M, P, L,  Q, max_min, U);
...
%     Coeff_Quality = 0.00000001*ones(size(SSM.y_typ_idx,2)+ size(SSM.u_typ_idx,2),bdb.nq,d1);
    ...
    input_const=minmax(i-PastHorizon:end,:);
    ...
    tic
    gams('QBMPC_final_state nlp=ipopt lo=0');
    time(i) = toc;
...
%     q_pred_traj(i,:) = q_pred(:);
    ...
    %%%% Epty GAMs variables %%%%
    clear data Coeff_Quality input_const;
