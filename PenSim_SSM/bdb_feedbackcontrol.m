...
fname = 'test2'; % file name to save database

nb = 60;  % Number of batches to generate
...
%            ODE_inputs(2) =  Nom_feed_rate1(k);
           ODE_inputs(2) =  abs(Nom_feed_rate1(k)+normrnd(0,0.05));
        else
            ODE_inputs(2) = 0;
        end
...
%         ODE_inputs(3)=Aeration_rate1(k);
        ODE_inputs(3)=Aeration_rate1(k)+normrnd(0,1);
...
%         ODE_inputs(4)=Agitator_power1(k);
        ODE_inputs(4)=Agitator_power1(k)++normrnd(0,5);
        
        ODE_inputs(6) = abs(Cooling_feed + normrnd(0, 0.05));
...
for b = 1:nb_PRBS  % for each batch
    ...
    for k = 1:nFE % for each sampling instant
        ...
        if(feed_started)
%            ODE_inputs(2) =  Nom_feed_rate1(k);
           ODE_inputs(2) =  Nom_feed_rate1(k)+normrnd(0,0.01);
        else
            ODE_inputs(2) = 0;
        end
...
%         ODE_inputs(3)=Aeration_rate1(k);
        ODE_inputs(3)=Aeration_rate1(k)+normrnd(0,0.3);
...
%         ODE_inputs(4)=Agitator_power1(k);
        ODE_inputs(4)=Agitator_power1(k)++normrnd(0,1);
        
        ODE_inputs(6) = Cooling_feed + normrnd(0, 0.001);
