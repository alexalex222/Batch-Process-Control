function [quality_result, percent_imp, x, y,q, u, u_all, handle, JMPC, time] = QBMPC_Penicillin(bdb, batch_data, batch_index, Q_model, Mult_model, qual_target)

if(nargin<=5)
    qual_target = bdb.qnom(:,end)';
end
...
% u = batch_data.unom'; %$$Would it be bdb.data.unom ?
u = bdb.data.unom';

u_all = zeros(bdb.nFE, bdb.nu, bdb.nFE);

% Wouldn't it be bdb.data.x and so on ?
...
% You'll have to figure out how the input constratins should look (should
% be the same as database generation)
umin = [290];
umax = [323];
...
% % % % Should we change these parameters ?
% This was to weight the input moves differnetly as time progressed.  We
% don't need this anymore. (Pick one input weight)
% weight_0 = .000005;
% weight_f = .000005;
% trans_start = 150;
% trans_end = 195;
...
for i = 1:bdb.nFE
 ...
 % This useed to is to pick a differnet input constratint for differnet
 % phase.  you don't need it anymore. (just use a constatnt input
 % constratint
%     for j = i:bdb.nFE+1
...
$$$%%% I don`t know if it works since i dont know the size of input_meas_means and input_meas_std
        input_const = [umin umax];
        
        % scale constraint
        input_const = (input_const - repmat(input_meas_means',1,2))./repmat(input_meas_std',1,2);
    
%     Why is there number 3 ?
    ...
    Tsqrd_limit = Q_model.T_lim_calc(.95); % this is something we can tune
    ...
    SPE_limit = Q_model.SPE_lim_calc(.9999); % we can probably tune this too
  ...
  %  subplot(2,1,1)
%  Why i:241 ? Is it general or just for this case ? (241 was the number of
%  sampling instances)
% Should we change the states that we want to plot ?

$$$% % % Should we select the input to be plotted ?
%x_out(:,4) was the input
    plot(i:bdb.nFE+1, x_out(:,4));
    hold on;
%     plot(batch_data.unom(1,:),'r');
    plot(bdb.data.unom(1,:),'r');
...
% I changed the ODE function which will be integrated

    %%%% Simulate to end of batch with the given input trajectory
    
    % DON"T WORRYO ABOUT THIS IF STATEMENT RIGHT NOW
    ...
    if(predict_to_end)
        ...
        for j = i:bdb.nFE
             %[T_out X_out] = ode23s(@methyl_methacrylate, [(j-1)*bdb.samt j*bdb.samt], X_out_complete(end,:), '', x(1,:), u_all(j,:,i) , 'state');
              [T_out X_out] = ode23s('pengsimv2_1', [(j-1)*bdb.samt j*bdb.samt], X_out_complete(end,:), '', x(1,:), u_all(j,:,i) , 'state');
            ...
            %aux = methyl_methacrylate((i)*bdb.samt,X_out_complete(end,:), x(1,:), u_all(j,:,i), 'output');
             aux = pengsimv2_1((i)*bdb.samt,X_out_complete(end,:), x(1,:), u_all(j,:,i), 'output');
        ...
        aux = pengsimv2_1((i)*bdb.samt,X_out_complete(end,:), x(1,:), u_all(end,:,i), 'output');
        q_act(i,:) = aux(1:4);
    end
...
% %     I changed the ODE function which will be integrated
    ...
    % this code should look a lot liek the database generation (except it uses
% the MPC input rather then the PI input)
    
    [T_out X_out] = ode23s('pengsimv2_1', [(i-1)*bdb.samt i*bdb.samt], x(i,:), '', x(1,:), u(i,1) , 'state');
    x(i+1,:) = X_out(end,:);

    %aux = methyl_methacrylate((i)*bdb.samt,x(i+1,:), x(1,:), u(i,1), 'output');
    aux = pengsimv2_1((i)*bdb.samt,x(i+1,:), x(1,:), u(i,1), 'output');
