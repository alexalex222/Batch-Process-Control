function [quality_result, q_old, x, y,q, u, JMPC, time] =...
          QBMPC_PenV1(bdb, batch_index, Q_model, Mult_model, qual_target)
%Quality Relevant Btach MPC function for Penicillin process
%   Input
%   bdb: batch data
%   index: the index of batch to be used in the MPC
%   Q_model: quality  model
%   Mult_model: the multimodel for process dynamics
%   qual_target: the desired quality values
%   Output
%   quality_result: the quality obtained by MPC
%   q_old: the quality obtained by PI controller
%   x: state of process
%   y: output of process
%   u: input of MPC
%   JMPC: objective function value of MPC
%   time: elapsed time

if(nargin<=4)
    qual_target = bdb.data.qnom(:,end)'.*[1 1.2 1.2];
end
...
% umin = [0;    7.9; 29; 0.05];
% umax = [0.09; 8.1; 31; 0.07 ];

minmax = zeros(bdb.nFE,2*bdb.nu);

for i = 1:4
    for j = 1: 600
        minmax(j,i) = min(bdb.data.u(i,j,:));
        minmax(j,i+4) = max(bdb.data.u(i,j,:));
    end
end

minmax(:,4) = minmax(:,4)-0.002;
minmax(:,8) = minmax(:,8)+0.002;
...
Clust_Cent = wgdxvar('FCMc', Clust_Cent, 2);
    ...
    U.name = 'U';
    for j = 1:size(Mult_model.Measurements{u_typ_idx},2)*2
        U.uels{j} = j;
    end
...
%       input_const=[umin umax];
    input_const=minmax(i:end,:);
      % Scale constraint
      input_const = (input_const - repmat(input_meas_means,size(minmax(i:end,:),1),2))./repmat(input_meas_std,size(minmax(i:end,:),1),2);
    ...
    if i<IndexQinter1
        ...
        for j = i:PredictionStep+1
            ...
            past_data = x(IndexQinter1,Q_model.x0_index);
            next_entry = size(Q_model.x0_index,2) +1;
            for j = IndexQinter1:i-1 % NOTE: this goes to i-1 because the current measurements will be handled in GAMs
                past_data(next_entry:next_entry+size(Q_model.x_index,2)-1) = x(j,Q_model.x_index);
                next_entry = next_entry + size(Q_model.x_index,2);
            ...
            for j = i-IndexQinter1+1:PredictionStep+1-IndexQinter1+1
                qual_coeffs(:,:,j-i+IndexQinter1) = Q_model.coeffs(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas,:);
            end
            
            %%%% Get Quality Model Scaling %%%%
            for j = i-IndexQinter1+1:PredictionStep+1-IndexQinter1+1
                q_meas_mean(j-i+IndexQinter1,:) = Q_model.x_mean(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas)';
                q_meas_std(j-i+IndexQinter1,:) = Q_model.x_std(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas)';
            end
            
        else 
            past_data = x(IndexQinter2,Q_model.x0_index);
            next_entry = size(Q_model.x0_index,2) +1;
            for j = IndexQinter2:i-1 % NOTE: this goes to i-1 because the current measurements will be handled in GAMs
                past_data(next_entry:next_entry+size(Q_model.x_index,2)-1) = x(j,Q_model.x_index);
                next_entry = next_entry + size(Q_model.x_index,2);
            ...
            for j = i-IndexQinter2+1:PredictionStep+1-IndexQinter2+1
                qual_coeffs(:,:,j-i+IndexQinter2) = Q_model.coeffs(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas,:);
            end
            
            %%%% Get Quality Model Scaling %%%%
            for j = i-IndexQinter2+1:PredictionStep+1-IndexQinter2+1
                q_meas_mean(j-i+IndexQinter2,:) = Q_model.x_mean(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas)';
                q_meas_std(j-i+IndexQinter2,:) = Q_model.x_std(num_initial+(j-1)*num_meas+1:num_initial+j*num_meas)';
            end
...
%        t_past_comp_gdx, w_star_reformed, Tsqrd_limit, past_data_gdx, w_star_past, SPE_limit, qual_lo);
    ...
    tic
    gams('QBMPC_Pen2 nlp=conopt lo=0');
    time(i) = toc;
    ...
    u(i:PredictionStep,1:4) = x_out(1:end-1,5:8);
