...
Q_model_Inter2.coeffs = beta_stored(:,:,nPC_idx);
Q_model_Inter2.y_index = desired_outputs;
Q_model_Inter2.x_index = [];
Q_model_Inter2.x0_index = desired_initial_states;
Q_model_Inter2.u_index = desired_inputs;
Q_model_Inter2.RMSE = RMSE;

Q_model_Inter2.f_index = [];
Q_model_Inter2.q_index = 1;
Q_model_Inter2.q_times = [];
Q_model_Inter2.nb_fit = nb_fit;
Q_model_Inter2.nb_val = nb_val;
Q_model_Inter2.nb_alt = (nb_test+nb_fit+nb_val) - nb_fit - nb_val;
Q_model_Inter2.nPC = PCs_to_try(nPC_idx);
Q_model_Inter2.sa = std(xs);
Q_model_Inter2.T95 = (nb_fit-1)*(nb_fit+1)*Q_model_Inter2.nPC/(nb_fit*(nb_fit-Q_model_Inter2.nPC))*finv(.95,Q_model_Inter2.nPC,nb_fit-Q_model_Inter2.nPC);
Q_model_Inter2.T99 = (nb_fit-1)*(nb_fit+1)*Q_model_Inter2.nPC/(nb_fit*(nb_fit-Q_model_Inter2.nPC))*finv(.99,Q_model_Inter2.nPC,nb_fit-Q_model_Inter2.nPC);
Q_model_Inter2.wstar = xl;
Q_model_Inter2.wstarAll = wstarAll;
Q_model_Inter2.Q = yl;
Q_model_Inter2.P = P;
Q_model_Inter2.W = W;
Q_model_Inter2.PAll = PAll;
Q_model_Inter2.OMEGA = Q_model_Inter2.wstar'*(x_fit_centred'*x_fit_centred)*Q_model_Inter2.wstar./(size(x_fit_centred,1)-1);

Q_model_Inter2.SPE95 = chi_95;
Q_model_Inter2.SPE99 = chi_99;

Q_model_Inter2.SPE_lim_calc = @(Prob) g*chi2inv(Prob, h);
Q_model_Inter2.T_lim_calc = @(Prob) (nb_fit-1)*(nb_fit+1)*Q_model_Inter2.nPC/(nb_fit*(nb_fit-Q_model_Inter2.nPC))*finv(Prob,Q_model_Inter2.nPC,nb_fit-Q_model_Inter2.nPC);

Q_model_Inter2.x_mean = x_fit_mu;
switch model_use
    case 'MultiMod'
        Q_model_Inter2.x_std = x_fit_sigma.*...
            [ones(1,size(Q_model_Inter2.x0_index,2))...
            ones(1,size(x_fit_sigma,2)-size(Q_model_Inter2.x0_index,2))*sqrt(size(x_fit_centred,2))];
    case 'MissingData'
        Q_model_Inter2.x_std = x_fit_sigma;
end
Q_model_Inter2.y_mean = qual_fit_mu;
Q_model_Inter2.y_std = qual_fit_sigma;



Q_model_Inter2.error = [0 0 0];

save(fname,'Q_model_Inter2');
%%
figure;
